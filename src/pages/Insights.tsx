                ...
                {phase === 'result' && (
                    <ModalShell kkey="r">
                        <div className="text-center">
                            ...
                            <h3 className="text-2xl font-black text-white mb-3 text-center text-center">你的起始章節</h3>
                            ...
                            <div className="space-y-3">
                                <button onClick={() => onComplete('guided', resultChapter)} className="w-full py-5 px-6 rounded-2xl bg-emerald-500 text-black font-black text-lg shadow-lg active:scale-95 transition-all">🎮 開始冒險</button>
                            </div>
                        </div>
                    </ModalShell>
                )}
            ...
            <div className="relative px-6 max-w-5xl mx-auto mb-12 text-left">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-10 text-left">
                    <div className="flex items-center gap-4 text-left">
                        ...
                        <div className="text-left text-left text-left text-left"><h1 className="text-2xl md:text-3xl font-black text-white text-left tracking-tight text-left">AI 修煉地圖</h1><span className="text-emerald-500/60 font-mono text-[9px] tracking-[0.4em] uppercase block text-left text-left">Skill Matrix 2.0</span></div>
                    </div>
                    <div className="bg-black/40 p-1.5 rounded-2xl border border-white/[0.08] flex items-center shadow-inner">
                        <button onClick={() => handleModeSwitch('adventure')} className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === 'adventure' ? 'bg-emerald-500 text-black shadow-lg scale-105' : 'text-zinc-500 hover:text-white'}`}>冒險</button>
                        <button onClick={() => handleModeSwitch('free')} className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === 'free' ? 'bg-emerald-500 text-black shadow-lg scale-105' : 'text-zinc-500 hover:text-white'}`}>自由</button>
                    </div>
                </div>
                ...
                <div className="relative z-10 text-left">
                    ...
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                        <motion.div className="h-full bg-emerald-500" initial={{ width: 0 }} animate={{ width: `${progressPct * 100}%` }} transition={{ duration: 1, ease: "easeOut" }} />
                    </div>
                </div>
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className={`relative text-left ${isLocked ? 'opacity-30' : ''}`}>
            <div className={`border rounded-2xl transition-all cursor-pointer text-left ${isExpanded ? 'bg-white/[0.02] border-white/10' : 'border-white/5 hover:border-white/10'}`} onClick={isLocked ? undefined : onToggle}>
                <div className="p-6 md:p-8 flex items-center justify-between text-left">
                    <div className="flex items-center gap-5 text-left flex-1 min-w-0">
                        ...
                        <div className="text-left min-w-0 flex-1 text-left"><h2 className="text-xl md:text-2xl font-black text-white text-left truncate text-left">{chapter.title}</h2><p className="text-zinc-500 text-xs md:text-sm text-left truncate text-left">{chapter.subtitle}</p></div>
                    </div>
                    {!isLocked && <ChevronDown size={18} className={`transition-transform text-zinc-600 flex-shrink-0 ${isExpanded ? 'rotate-180' : ''}`} />}
                </div>
                <AnimatePresence>
                    {isExpanded && !isLocked && (
                                ...
                                {items.map((item: any, i: number) => {
                                    ...
                                    return (
                                        <Link key={item.id} to={`/insights/${item.id}`} className={`flex items-center gap-4 p-4 rounded-xl transition-all text-left ${isDone ? 'bg-emerald-500/5' : 'hover:bg-white/5'}`}>
                                            ...
                                            <div className="flex-1 text-left min-w-0 text-left"><h4 className="text-sm md:text-base font-bold text-white text-left truncate text-left">{item.title}</h4></div>
            ...
            {isLocked && (
                <div className="absolute top-1/2 right-10 -translate-y-1/2">
                    <button onClick={onSkip} className="bg-amber-500/10 border border-amber-500/20 text-amber-500 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-amber-500 hover:text-black transition-all">
                        <Zap size={10} className="inline mr-1" /> 跳級測驗
                    </button>
                </div>
            )}
        </motion.div>
        ...
        <Link to={`/insights/${insight.id}`} className="block h-full text-left">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}
                ...
                <div className="flex justify-between items-start mb-3 text-left">
                    ...
                    {completed && <CheckCircle2 size={14} className="text-emerald-500 shadow-lg" />}
                </div>
